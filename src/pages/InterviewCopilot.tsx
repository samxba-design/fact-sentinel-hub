...
const InterviewCopilot: React.FC = () => {
  ...
  const handleQuestion = useCallback(
    async (question: string, fullTranscript: string) => {
      ...
      await generateResponse({
        ...
        context,
        history,
        previousResponses: prevResponseTexts,
      });
      ...
      switch (e.key) {
        case ' ':
          if (e.ctrlKey || e.metaKey) {
            e.preventDefault();
            if (phase === 'listening' || phase === 'capturing' || phase === 'generating') {
              stopSession();
            } else {
              startSession();
            }
  ...
  // Minimized mode — discreet floating bar
        ...
        <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2 px-4 py-2 rounded-full bg-background border border-border shadow-lg">
          ...
          {isStreaming && (
            <span className="text-[10px] text-primary animate-pulse">
              +{(streamingText.match(/[.!?]/g) || []).length} sentences
            </span>
          )}
          <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => setMinimized(false)}>
            <Minimize2 className="w-3.5 h-3.5" />
          </Button>
          <Button variant="destructive" size="sm" className="h-7 text-[10px]" onClick={stopSession}>
    ...
    <div className="flex flex-col h-[calc(100vh-0rem)] -m-6">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-card/50 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {autoGenerate ? <Eye className="w-3 h-3 text-green-500" /> : <EyeOff className="w-3 h-3" />}
            Auto
          </Button>
          ...
          {isActive && (
            <Button
              ...
              title="Minimize to discreet bar (Ctrl+H)"
            >
          ...
          >
            ...
            {showSettings ? 'Hide' : 'Context'}
          </Button>

          {isActive ? (
            <Button variant="destructive" size="sm" className="h-8 gap-2" onClick={stopSession}>
              <Square className="w-3.5 h-3.5" /> Stop
            </Button>
          ) : (
            <Button variant="default" size="sm" className="h-8 gap-2 bg-green-600 hover:bg-green-700" onClick={startSession}>
      ...
      <div className="flex items-center gap-3 px-4 py-1.5 bg-muted/30 border-b border-border text-[10px] font-mono shrink-0">
        <span className="flex items-center gap-1.5">
          ...
          />
          {phase === 'idle' && 'Ready — Set up context, then Start'}
          {phase === 'setup' && 'Configure interview context'}
          {phase === 'capturing' && 'Capturing system audio...'}
          {phase === 'listening' && isStreaming ? 'Streaming response...' : 'Listening & transcribing...'}
          {phase === 'generating' && 'Generating response...'}
          {phase === 'error' && 'Error — check console'}
        </span>
        <span className="text-muted-foreground truncate">
          {context.jobTitle
            ? `🎯 ${context.jobTitle} @ ${context.companyName || '...'} (${context.interviewStyle})`
            : '⚠️ Configure interview context in the sidebar'}
        </span>
      ...
      {(audioError || transcribeError) && (
        <div className="flex items-center gap-2 px-4 py-2 bg-destructive/10 border-t border-destructive/20 text-destructive text-xs shrink-0">
          ...
          <Button
            ...
            onClick={() => { stopSession(); setTimeout(startSession, 800); }}
          >
            Retry
          </Button>
        </div>
      ...
      <div className="flex items-center gap-3 px-4 py-1.5 bg-muted/20 border-t border-border text-[10px] text-muted-foreground shrink-0">
        ...
        <kbd className="px-1 py-0.5 rounded bg-muted border border-border text-[9px]">Ctrl+Space</kbd>
        <span>Start/Stop</span>
        <kbd className="px-1 py-0.5 rounded bg-muted border border-border text-[9px]">Ctrl+R</kbd>
        <span>Regenerate</span>
        <kbd className="px-1 py-0.5 rounded bg-muted border border-border text-[9px]">Ctrl+T</kbd>
        <span>Manual trigger</span>
        <kbd className="px-1 py-0.5 rounded bg-muted border border-border text-[9px]">Ctrl+H</kbd>
        <span>Hide</span>
        <kbd className="px-1 py-0.5 rounded bg-muted border border-border text-[9px]">Esc</kbd>
        <span>Exit hide</span>
      </div>
    </div>
  );
