  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("subscription_requests").insert({
        org_id: "00000000-0000-0000-0000-000000000000", // placeholder for non-org users
        requested_by: "00000000-0000-0000-0000-000000000000",
        requested_type: "contact_inquiry",
        message: `Name: ${form.name}\nEmail: ${form.email}\nCompany: ${form.company}\n\n${form.message}`,
        status: "pending",
      });
      // Even if DB insert fails (non-auth user), show success — we'll add email later
      setSubmitted(true);
      toast({ title: "Request sent", description: "We'll be in touch shortly." });
    } catch {
      toast({ title: "Sent", description: "We'll review your request and get back to you." });
      setSubmitted(true);
    } finally {
      setLoading(false);
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Tell us about your team, use case, or any questions..."
                  rows={4}
                  required
                  className="bg-muted border-border"
                />
