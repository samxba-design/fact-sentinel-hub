...
import AutoGenerateDialog from "@/components/templates/AutoGenerateDialog";
...
export default function ApprovedTemplatesPage() {
    ...
    try {
      if (editingTemplate) {
        const { error } = await supabase.from("approved_templates").update({
          name: data.name, template_text: data.template_text, scenario_type: data.scenario_type,
          tone: data.tone, platform_length: data.platform_length, status: data.status,
        }).eq("id", editingTemplate.id);
        if (error) throw error;
        toast({ title: "Template updated" });
      } else {
        const { error } = await supabase.from("approved_templates").insert({
          org_id: currentOrg.id, name: data.name, template_text: data.template_text,
          scenario_type: data.scenario_type, tone: data.tone, platform_length: data.platform_length, status: data.status,
        });
        if (error) throw error;
  ...
  const filtered = search
    ? templates.filter(t => t.name.toLowerCase().includes(search.toLowerCase()) || t.scenario_type?.toLowerCase().includes(search.toLowerCase()))
    : templates;
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <Card className="bg-card border-border p-8 text-center space-y-3">
            <p className="text-sm text-muted-foreground">No templates yet. Create one manually or use Auto-Generate.</p>
            <Button variant="outline" onClick={() => setAutoGenOpen(true)}>
              <Sparkles className="h-4 w-4 mr-2" /> Auto-Generate from Facts
            </Button>
          </Card>
        ) : (
          filtered.map(t => {
      ...
      <AutoGenerateDialog open={autoGenOpen} onOpenChange={setAutoGenOpen} onSaved={fetchTemplates} />
